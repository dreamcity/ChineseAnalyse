...
map<string , int> WordModel::getStateMap()
{
	return statemap;
}
map<string , int> WordModel::getWordMap()
{
	return wordmap;
}
...
void fileClean(const char* inputfile, const char* outputfile)
{
	...
	while(getline(infile, strtmp, '\n'))
	{
		...
		while(strtmp.find('[') != string::npos )
		{
			...
			std::vector<string> vstr = split(str1," ");
...
void WordModel::filePreprocess(const char* inputfile, const char* outputfile)
{
...
// WordModel::getMapData(const char* inputfile,map<string, int>& statemap, 
// 					  map<string,	int>& wordmap, const char* outputfile)
void WordModel::getMapData(const char* inputfile, const char* outputfile)
{
...
//void WordModel::getIniPro(const char* inputfile, map<string, int> statemap)
void WordModel::getIniPro(const char* inputfile)
{
...
// void WordModel::getTranPro(const char* inputfile, map<string, int> statemap)
void WordModel::getTranPro(const char* inputfile)
{
	string strtmp;
	ifstream infile(inputfile);
	while(getline(infile, strtmp, '\n'))
...
// void WordModel::getConPro(const char* inputfile, map<string, int> wordmap, 
// 				map<string, int>& wordmap)
void WordModel::getConPro(const char* inputfile)
{
	string strtmp; //用于保存从语料库中读入的每一行
	ifstream infile(inputfile);
	while(getline(infile, strtmp, '\n'))
...
void WordModel::saveModelData(const char* outputfile)
{
	ofstream outfile;
	outfile.open(outputfile);
	outfile << M <<endl;
